...
export async function generateLinkedInProfile(formData: ProfileFormValues): Promise<string> {
  try {
    ...
    const prompt = formData.language === 'es' 
      ? `Genera una sección "Acerca de" profesional para LinkedIn con el siguiente tono: ${formData.toneOfVoice}.
        ...
        con ${formData.yearsOfExperience} años de experiencia. Utiliza las siguientes tecnologías y herramientas: ${formData.technologiesUsed}.
    ...
    const response = await openai.chat.completions.create({
      model: "gpt-3.5-turbo",
      messages: [
        { role: "system", content: `You are a professional copywriter specialized in LinkedIn profiles. You write in ${languageText}.` },
